...
- [2 基础部分](#2-基础部分)
  ...
  - [2.4 函数重载, inline 函数, 参数带默认值的函数](#24-函数重载-inline-函数-参数带默认值的函数)
    - [2.4.1 函数重载](#241-函数重载)
...
- [3 面向对象](#3-面向对象)
  ...
  - [3.3 初始化列表](#33-初始化列表)
  - [3.4 普通，static，const 三类成员方法](#34-普通staticconst-三类成员方法)
...
- [7 继承与多态](#7-继承与多态)
  ...
  - [7.4 虚函数、静态绑定和动态绑定](#74-虚函数静态绑定和动态绑定)
  - [7.5 虚析构函数](#75-虚析构函数)
  - [7.6 动态绑定](#76-动态绑定)
  - [7.7 多态](#77-多态)
  - [7.8 抽象类](#78-抽象类)
  - [7.9 继承多态常见面试题分享](#79-继承多态常见面试题分享)
    - [7.9.1 问题一](#791-问题一)
    - [7.9.2 问题二](#792-问题二)
    - [7.9.3 问题三](#793-问题三)
    - [7.9.4 问题四](#794-问题四)
  - [7.10 四种语言级别的类型转换方式](#710-四种语言级别的类型转换方式)
  - [7.11 虚基类和虚继承](#711-虚基类和虚继承)
  - [7.12 菱形继承](#712-菱形继承)
...
> inline 只是建议编译器把函数处理为 内联函数，并不是所有的 inline 都会被编译器内联，例如递归
...
[string 容器](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#331-string%E5%AE%B9%E5%99%A8)
...
- [构造和析构顺序](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2564-%E7%BB%A7%E6%89%BF%E4%B8%AD%E6%9E%84%E9%80%A0%E5%92%8C%E6%9E%90%E6%9E%84%E9%A1%BA%E5%BA%8F)
...
- [多态](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2573-%E7%BA%AF%E8%99%9A%E5%87%BD%E6%95%B0%E5%92%8C%E6%8A%BD%E8%B1%A1%E7%B1%BB)
  - 静态绑定
  - 动态绑定
  - 虚函数
...
>
>   - 一个类中定义了虚函数，那么编译阶段，编译器给这个类类型产生一个唯一的 `vftable` 虚函数表，虚函数表中存放的是 RTTI(run-time type information) 指针和虚函数的地址
>
>   - 当程序运行时，每一张虚函数表都会加载到内存的 `.rodata` 段中
>
>   - 一个类中定义了虚函数，那么这个类定义的对象，其运行时，内存中开始部分，多存储一个 `vfptr` 虚函数指针，指向相应类型的虚函数表 `vftable`。一个类型定义的 n 个对象，它们的 `vfptr` 指向的都是同一个虚函数表
>
>   - **一个类里面虚函数的个数，不影响对象内存大小（`vfptr`），影响的是虚函数表的大小**
>   - 如果派生类中的方法，和基类继承来的某个方法，返回值、函数名、参数列表都相同，而且基类的方法是 `virtual` 的，那么派生类的方法也是 `virtual` 的，这样就实现了**覆盖**，否则就是**隐藏**

```cpp
class Base {
public:
  void show() { std::cout << "Base class show function" << std::endl; }
  void show(int a) {
...
class Derived : public Base {
public:
  void show() { std::cout << "Derived class show function" << std::endl; }
private:
  int m_b;
};
...
int main() {
  ...
  Base *d = &b;
  // 静态(编译时期)的绑定
  d->show();   // Base class show function
  d->show(10); // Base class show function with int
  // 如果发现 show() 是虚函数，那么就会进行动态绑定

  std::cout << sizeof(Base) << std::endl;    // 4
  std::cout << sizeof(Derived) << std::endl; // 8

  std::cout << typeid(d).name() << std::endl; // P4Base
  std::cout << typeid(*d).name() << std::endl; // 4Base
...
- 问题二：什么时候需要虚析构函数？
  - **基类的指针（引用）指向堆上 `new` 出来的派生类对象时，`delete` 时，需要调用派生类的析构函数**
  - [虚析构函数](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2575-%E8%99%9A%E6%9E%90%E6%9E%84%E5%92%8C%E7%BA%AF%E8%99%9A%E6%9E%90%E6%9E%84)，析构函数调用时，对象是存在的
...
- [抽象类](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2573-%E7%BA%AF%E8%99%9A%E5%87%BD%E6%95%B0%E5%92%8C%E6%8A%BD%E8%B1%A1%E7%B1%BB)
  - 抽象类不能再实例划对象，**但是可以定义指针和引用**
...
- `const_cast` : 去掉常量属性的转换
- `static_cast` : 提供编译器认为安全的类型转换。没有任何联系的类型之间的转换会被拒绝。**最常用**
- `reinterpret_cast` : 类似于 C 语言中的强制类型转换，不安全
...
int main() {
  ...
  const int a = 10;
  int *p1 = (int *)&a;
  // double *p1 = (double *)&a; // 访问非法内存，可以通过编译

  int *p2 = const_cast<int *>(&a);
  // double *p2 = const_cast<double *>(&a); // invalid 'const_cast' from type
  'const int*' to type 'double*'
  ...
  */
  ...
  // int *p = nullptr;
  // short *b = static_cast<short *>(p); // 类型转换无效
  return 0;
  */
}
```
...
public:
  ...
  // Derived2 新功能的 API 接口
  void driver02func() { cout << "Derived2::driver02func()" << endl; }
};

void showFunc(Base *p) {
  // dynamic_cast 会检查 p 是否是 Derived2 类型的对象
  // 如果是，则返回 Derived2 类型的指针
  // 如果不是，则返回 nullptr
  Derived2 *pd2 = dynamic_cast<Derived2 *>(p);
  pd2 ? pd2->driver02func() : p->func();
}
...
- [多继承](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2567-%E5%A4%9A%E7%BB%A7%E6%89%BF%E8%AF%AD%E6%B3%95)
- [菱形继承](https://github.com/Corner430/study-notes/blob/main/cpp/cpp%E5%85%A5%E9%97%A8%E7%AC%94%E8%AE%B0.md#2568-%E8%8F%B1%E5%BD%A2%E7%BB%A7%E6%89%BF)
