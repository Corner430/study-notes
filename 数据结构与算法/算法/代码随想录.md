...
```go
func findLeftBound(nums []int, target int, left int, right int) int {
    for left <= right {
        mid := left + (right - left) / 2
        if nums[mid] >= target {
            right = mid - 1
...
func findRightBound(nums []int, target int, left int, right int) int {
    for left <= right {
        mid := left + (right - left) / 2
        if nums[mid] > target {
            right = mid - 1
...
func searchRange(nums []int, target int) []int {
    leftBound := findLeftBound(nums, target, 0, len(nums) - 1)
    if leftBound == -1 {
        return []int{-1, -1}
    }
    return []int{leftBound, findRightBound(nums, target, leftBound, len(nums) - 1)}
}
```
...
private:
  vector<int> path;
  vector<vector<int>> res;
  void backtracking(int k, int n, const int startIndex) {
    if (path.size() == k) {
  ...
  int trap(vector<int> &height) {
    ...
    for (int i = 1; i < height.size(); ++i) {
      while (!st.empty() && height[i] > height[st.top()]) {
        ...
        st.pop();
        if (!st.empty())
          sum += (min(height[st.top()], height[i]) - height[cur]) *
                 (i - st.top() - 1);
      }
      st.push(i);
    }
